...
\centerline{\bf Towers of Hanoi (JavaScript)}
...
/* {\hrulefill\ hanoi.js \ \hrulefill} */


const ARRAY = 8;                    /* {\ disc の数 \hfill} */

const disc = [                      /* {\ disc に関するデータの置き場所 \hfill} */
    new Array(ARRAY).fill(0),
    new Array(ARRAY).fill(0)
];

function initArray() {              /* {\ disc に関するデータの初期化 \hfill} */
...
let counter = 0;                    /* {\ 移動回数カウンタ \hfill} */

function printResult() {            /* {\ 結果の表示 \hfill} */
    counter++;
    console.log(`---#${counter}---`);
    for (let i = 0; i <= 2; i++) {
        let line = `[${i}] `;
...
const ptr = [0, 0, 0];              /* {\ disc 移動用ポインタ（インデックス） \hfill} */

function moveOneDisc(i, j) {        /* {\ 1枚の disc を pole $i$ から
                                    pole $j$ に移動する \hfill} */
    ptr[i]--;
...
function moveDiscs(n, i, j, k) {    /* {\ 上から $n$ 枚目までの disc を、pole $i$ から pole $j$ に \hfill} */
...
initArray();
moveDiscs(ARRAY, 0, 1, 2);          /* {\ {\tt ARRAY} 枚の disc を pole 0 から pole 1 に pole 2\hfill} */
                                    /* {\ を経由して、移動する \hfill} */
