...
\centerline{\bf Towers of Hanoi (Kotlin)}
...
/* {\hrulefill\ hanoi.kt \ \hrulefill} */


const val ARRAY = 8			/* {\ disc の数 \hfill} */

val disc = Array(3) { IntArray(ARRAY) }	/* {\ disc に関するデータの置き場所
					   \hfill} */

fun initArray() {			/* {\ disc に関するデータの初期化
					   \hfill} */
    for (j in 0 until ARRAY) {
        disc[0][j] = ARRAY - j
        disc[1][j] = 0
        disc[2][j] = 0
    }
...
var counter = 0				/* {\ 移動回数カウンタ \hfill} */

fun printResult() {			/* {\ 結果の表示 \hfill} */
    counter++
    println("---#$counter---")
    for (i in 0..2) {
        print("[$i] ")
...
val ptr = intArrayOf(0, 0, 0)		/* {\ disc 移動用ポインタ（インデックス）
					   \hfill} */

fun moveOneDisc(i: Int, j: Int) {	/* {\ 1枚の disc を pole $i$ から
					   pole $j$ に移動する \hfill} */
    ptr[i]--
...
fun moveDiscs(n: Int, i: Int, j: Int, k: Int) {
					/* {\ 上から $n$ 枚目までの disc
					...
					/* {\ pole $k$ を経由して、移動する
					   \hfill} */
    if (n >= 1) {
        moveDiscs(n - 1, i, k, j)	/* {\ 関数 {\tt moveDiscs()}
					   の中で、さらに自分自身 \hfill} */
        moveOneDisc(i, j)		/* {\ {\tt moveDiscs()} が使われ
					   ている。このような \hfill} */
        printResult()			/* {\ 手法は、「再帰的呼びだし」
					   といわれる。 \hfill} */
        moveDiscs(n - 1, k, j, i)
    }
}
...
fun main() {
    ...
    initArray()
    moveDiscs(ARRAY, 0, 1, 2)		/* {\ {\tt ARRAY} 枚の disc を
