...
\centerline{\bf Towers of Hanoi (TypeScript)}
...
/* {\hrulefill\ hanoi.ts \ \hrulefill} */


const ARRAY: number = 8;            /* {\ disc の数 \hfill} */

const disc: number[][] = [          /* {\ disc に関するデータの置き場所 \hfill} */
    new Array(ARRAY).fill(0),
    new Array(ARRAY).fill(0)
];

function initArray(): void {        /* {\ disc に関するデータの初期化 \hfill} */
...
let counter: number = 0;            /* {\ 移動回数カウンタ \hfill} */

function printResult(): void {      /* {\ 結果の表示 \hfill} */
    counter++;
    console.log(`---#${counter}---`);
    for (let i = 0; i <= 2; i++) {
        let line: string = `[${i}] `;
...
const ptr: number[] = [0, 0, 0];    /* {\ disc 移動用ポインタ（インデックス）\hfill} */

function moveOneDisc(i: number, j: number): void {
                                    /* {\ 1枚の disc を pole $i$ から \hfill} */
                                    /* {\ pole $j$ に移動する \hfill} */
    ptr[i]--;
...
function moveDiscs(n: number, i: number,
    j: number, k: number): void {
                                    /* {\ 上から $n$ 枚目までの disc を、pole $i$ から \hfill} */
                                    /* {\ pole $j$ に pole $k$ を経由して、移動する \hfill} */
    if (n >= 1) {
...
initArray();
moveDiscs(ARRAY, 0, 1, 2);      /* {\ {\tt ARRAY} 枚の disc を \hfill} */
                                /* {\ pole 0 から pole 1 に pole 2 \hfill} */
                                /* {\ を経由して、移動する \hfill} */
