...
#
# {\ \centerline{\bf Towers of Hanoi (Ruby)} }
#
# {\ \begin{quote} }
# {\ \noindent }
...
# {\hrulefill\ hanoi.rb \ \hrulefill}


ARRAY = 8                               # {\ disc の数 \hfill }

$disc = Array.new(3) { Array.new(ARRAY, 0) }
...
def move_discs(n, i, j, k)              # {\ 上から $n$ 枚目までの disc を \hfill }
                                        # {\ pole $i$ から pole $j$ に \hfill }
                                        # {\ pole $k$ を経由して移動する \hfill }
    if n >= 1
        move_discs(n-1, i, k, j)
                                        # {\ 関数 {\tt move\_discs()} の中で \hfill }
                                        # {\ さらに自分自身が使われている。 \hfill }
        move_one_disc(i, j)             # {\ このような手法は「再帰的呼び \hfill }
        print_result                    # {\ だし」といわれる。 \hfill }
...
init_array
move_discs(ARRAY, 0, 1, 2)              # {\ {\tt ARRAY} 枚の disc を \hfill }
                                        # {\ pole 0 から pole 1 に \hfill }
                                        # {\ pole 2 を経由して移動する \hfill }
